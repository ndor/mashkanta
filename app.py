...
def set_amortizations():
    st.sidebar.segmented_control(
        ...
        default=params.PARAMETER_LANG_MAP[st.session_state['language_key']]['input']['amortizations'][2],
...
def set_prime_portion():
    st.sidebar.segmented_control(
        ...
        default=params.PARAMETER_LANG_MAP[st.session_state['language_key']]['input']['prime_portion'][1],
...
def set_net_monthly_income():
    st.sidebar.number_input(
        ...
        min_value=int(3 * params.min_monthly_payment(st.session_state['asset_cost'], st.session_state['capital'])), # 30 years of net monthly income
        max_value=int((st.session_state['asset_cost'] - st.session_state['capital']) / (12 * 2)), value='min', step=10, placeholder=None, key='net_monthly_income',
        format=params.NUMERICAL_FORMAT)
...
#         min_value=int(params.min_monthly_payment(st.session_state['asset_cost'], st.session_state['capital'])), # 30 years of net monthly income
#         max_value=st.session_state['net_monthly_income'], value='min', step=10, placeholder=None, key='max_monthly_payment',
#         format=params.NUMERICAL_FORMAT)


def set_max_monthly_payment():
    st.session_state['max_monthly_payment'] = st.session_state['net_monthly_income'] // 3
    st.sidebar.markdown(f"{params.PARAMETER_LANG_MAP[st.session_state['language_key']]['input_titles']['max_monthly_payment']} : **{st.session_state['max_monthly_payment']}**")
...
def project_about():
    for k in sorted(params.PARAMETER_LANG_MAP[st.session_state['language_key']]['about'].keys()):
        st.image(params.PARAMETER_LANG_MAP[st.session_state['language_key']]['about'][k]['image'])
