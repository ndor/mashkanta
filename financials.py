...
def get_madad(initial_value=params.MADAD_INITIAL_VALUE,
              initial_steady_period_months_duration=params.INITIAL_STEADY_PERIOD_MONTHS_DURATION,
              steady_ramp_months_duration=params.STEADY_RAMP_MONTHS_DURATION,
              long_range_centerline=params.LONG_RANGE_CENTERLINE):

    madad_0 = initial_value * np.ones(initial_steady_period_months_duration, dtype='float32')
    ...
    madad_2 = madad_1[-1] * np.ones(params.MAX_DURATION)
    madad = np.concatenate([madad_0, madad_1, madad_2]).astype('float32')
    return madad


def get_prime(initial_value=params.PRIME_INITIAL_VALUE,
              initial_steady_period_months_duration=params.INITIAL_STEADY_PERIOD_MONTHS_DURATION,
              steady_ramp_months_duration=params.STEADY_RAMP_MONTHS_DURATION,
              long_range_centerline=params.LONG_RANGE_CENTERLINE,
              banks_margine=params.BANKS_MARGINE,
              wavewlwngthy=10,
              amplitude=0.025):
    try:
        ...
        pass
    prime_0 = initial_value * np.ones(initial_steady_period_months_duration, dtype='float32') + banks_margine
    ...
    prime = np.concatenate([prime_0, prime_1, prime_2]).astype('float32')
    return prime
...
def get_spitzer_amortization(monthly_rate_array: np.ndarray, duration: int, principal: int) -> dict:
    ...
    return {'pmt': pmt.astype('float32'), 'ipmt': ipmt.astype('float32'), 'ppmt': ppmt.astype('float32')}
...
def risk_rating(funding_rate: float, is_married_couple=False) -> dict:
    un_married_rate = 1.
    if not is_married_couple:
        un_married_rate += params.UNMARRIED_ADDED_RISK
...
def update_yearly_to_monthly_rates_with_risk(funding_rate: float, is_married_couple=False) -> dict:
    ...
    fixed_rate = np.ones_like(monthly_changing_yearly_MADAD) * params.FIXED_VALUE
    ...
    return {'fixed_rate': yearly_rate_to_monthly(fixed_rate).astype('float32'),
            'madad_rate': yearly_rate_to_monthly(madad_rate).astype('float32'),
            'prime_rate': yearly_rate_to_monthly(prime_rate).astype('float32')}
...
if __name__ == '__main__':
    ...
    d = update_yearly_to_monthly_rates_with_risk(funding_rate, False)
